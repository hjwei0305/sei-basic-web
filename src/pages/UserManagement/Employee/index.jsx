      ...
      <PageWrapper className={cls(styles['container-box'])}>
        <Spin spinning={loading.global} wrapperClassName={cls('spin-wrapper')}>
          <div
            style={{
              height: '100%',
              display: !showEmployeeConfig && !showCopyConfig ? '' : 'none',
            }}
          >
            <CascadeLayout title={['组织机构', currNode && currNode.name]} layout={[8, 16]}>
              ...
              {currNode ? (
                <TablePanel slotClassName={cls('table-slot-container')} slot="right" />
              ) : (
                <Empty
                  slot="right"
          ...
          {rowData && showEmployeeConfig ? (
            <EmployeeConfig style={{ display: showEmployeeConfig ? '' : 'none' }} />
          ) : null}
          {rowData && showCopyConfig ? (
            <CopyConfig style={{ display: showCopyConfig ? '' : 'none' }} />
          ) : null}
        </Spin>
      </PageWrapper>
    );

...
import { MoneyInput, ListCard, BannerTitle } from 'suid';
...
class Users extends Component {
  ...
  componentDidUpdate(preProps) {
    const { showUser, topNum } = this.props;
    if (!isEqual(preProps.showUser, showUser) && !showUser) {
      this.setState({ topNum });
    }
  }

  handlerClose = () => {
    const { closeUser } = this.props;
    if (closeUser) {
      closeUser();
    }
  };
  ...
  render() {
    const { topNum } = this.state;
    const { showUser, period, feature } = this.props;
    const listCardProps = {
      showSearch: false,
      showArrow: false,
      itemField: {
        avatar: ({ index }) => <Tag>{index + 1}</Tag>,
        title: item => `${item.userName}(${item.userAccount})`,
        description: item => `最近访问时间: ${item.accessTime}`,
        extra: item => <span className="count-tag">{item.countNum}</span>,
      },
      store: {
        url: `${SERVER_PATH}/sei-auth/accessRecord/getUsersByFeature`,
      },
      cascadeParams: {
    ...
    return (
      <Drawer
        ...
        placement="right"
        visible={showUser}
        title={<BannerTitle title={`${get(feature, 'feature')}`} subTitle="访问的用户排名" />}
        className={cls(styles['user-box'])}
